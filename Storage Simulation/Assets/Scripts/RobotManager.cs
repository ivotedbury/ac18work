...
public class RobotManager : MonoBehaviour
{
    ...
    public void InitialiseRobots(Structure _structure)
    {
        ...
        for (int i = 0; i < numberOfRobots; i++)
        {
            ...
            newRobot.InitialiseRobot(structure);
            ...
            newRobot.AssignTask("Pick up", structure.nodesArray[Random.Range(0, 9), 0, Random.Range(0, 9)]);
            newRobot.currentAction = new RobotAction(new RobotState(newRobot.transform.position, newRobot.transform.rotation, 0), new RobotState(structure.nodesArray[Random.Range(0,9),0, Random.Range(0, 9)].transform.position, Quaternion.Euler(0,90,0), Constants.RAISED_PLATFORM_HEIGHT.y), newRobot.currentNode);
